-- PostgreSQL Database Schema for Missing Persons System
-- Drop tables if they exist (for clean setup)
DROP TABLE IF EXISTS preliminary_uidb_reports CASCADE;
DROP TABLE IF EXISTS unidentified_bodies CASCADE;
...
CREATE TABLE missing_persons (
    ...
    reported_date TIMESTAMP NOT NULL,  -- Date and time the missing report was filed
    name VARCHAR(100),  -- Full name of the missing person (if known)
    age INTEGER,  -- Estimated or known age
    gender VARCHAR(20) CHECK (gender IN ('Male', 'Female', 'Other', 'Unknown')),  -- Gender
    ...
    last_seen_date TIMESTAMP,  -- Date and time last seen
    ...
    status VARCHAR(20) CHECK (status IN ('Open', 'Matched', 'Closed')) DEFAULT 'Open',  -- Case status for analytics
...
CREATE TABLE unidentified_bodies (
    ...
    reported_date TIMESTAMP NOT NULL,  -- Date and time the case was officially filed
    found_date TIMESTAMP NOT NULL,  -- Date and time the body was found
    postmortem_date TIMESTAMP,  -- Date postmortem was conducted
    estimated_age INTEGER,  -- Age determined from postmortem
    gender VARCHAR(20) CHECK (gender IN ('Male', 'Female', 'Other', 'Unknown')),  -- Gender determined from postmortem
    height_cm INTEGER,  -- Height measured during postmortem
    ...
    status VARCHAR(20) CHECK (status IN ('Open', 'Matched', 'Closed')) DEFAULT 'Open',  -- Case status
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,  -- Record creation time
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP  -- Last update time
);
...
CREATE TABLE preliminary_uidb_reports (
    ...
    reported_date TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,  -- Date and time the initial report was filed
    found_date TIMESTAMP NOT NULL,  -- Date and time the body was found
    estimated_age INTEGER,  -- Rough estimated age
    gender VARCHAR(20) CHECK (gender IN ('Male', 'Female', 'Other', 'Unknown')),  -- Apparent gender
    height_cm INTEGER,  -- Rough estimated height in centimeters
    ...
    status VARCHAR(20) CHECK (status IN ('Pending', 'Processed', 'Archived')) DEFAULT 'Pending',  -- Status: Pending postmortem, Processed (moved to main table)
    uidb_id INTEGER REFERENCES unidentified_bodies(id),  -- Foreign key to link to full UIDB entry once postmortem is done (nullable)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,  -- Record creation time
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP  -- Last update time
);
...
CREATE TRIGGER update_unidentified_bodies_updated_at BEFORE UPDATE ON unidentified_bodies
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_preliminary_uidb_updated_at BEFORE UPDATE ON preliminary_uidb_reports
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
